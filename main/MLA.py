...
def group(
    ...
    if len(xs) == 0:
        ...
        out = torch.stack(xs) #2*节点数量*嵌入长度
        if out.numel() == 0:
            return out.view(0, out.size(-1)), None
        attn_score = (q * torch.tanh(k_lin(out)).mean(1)).sum(-1)
        attn = F.softmax(attn_score, dim=0)
        # print(attn.shape) #2
        # print(attn.view(num_edge_types, 1, -1).shape) #num_edge_types*1*1
        out = torch.sum(attn.view(num_edge_types, 1, -1) * out, dim=0) #节点数量*嵌入长度
        return out, attn
...
class HANConv(MessagePassing):
    ...
    def forward(
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            edge_type = '__'.join(edge_type)
            lin_src = self.lin_src[edge_type] #1*heads*dim/head
            ...
            out = self.propagate(edge_index, edge_type=edge_type,x=(x_src, x_dst),
    ...
    def message(self,edge_type, x_j: Tensor, alpha_i: Tensor, alpha_j: Tensor,
